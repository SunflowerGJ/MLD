...
- (void)viewDidLoad {
    ...
    _toolbar = [[UIToolbar alloc] initWithFrame:[self frameForToolbarAtOrientation:interfaceOrientation]];
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    if ([self areControlsHidden] && !hidden && animated) {
        ...
        // Toolbar
        UIInterfaceOrientation interfaceOrientation = [[UIApplication sharedApplication] statusBarOrientation];
        _toolbar.frame = CGRectOffset([self frameForToolbarAtOrientation:interfaceOrientation], 0, animatonOffset);
...
- (void)actionButtonPressed:(id)sender {
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
            ...
            [self.activityViewController setCompletionWithItemsHandler:^(NSString *activityType, BOOL completed,NSArray *returnedItems,NSError *activiteError){
