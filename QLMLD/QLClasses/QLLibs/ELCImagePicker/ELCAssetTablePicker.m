...
@property (nonatomic, assign) NSInteger rowShow; //一次最多提取图片数量
@property (nonatomic, strong) NSMutableArray *elcAssets;
@property (nonatomic, strong) UILabel *selectedAssetsLabel;
...
- (void)viewDidLoad
{
    ...
    self.rowShow = MAX(self.view.bounds.size.height/80, 300) ;
...
{
    @autoreleasepool {
//#warning 修改：将图片分批并按时间倒序显示
        DebugLog(@"---1");
        [self.elcAssets removeAllObjects];
        DebugLog(@"---2");
        NSInteger numShow = self.rowShow;
        NSInteger numAssets = self.assetGroup.numberOfAssets;
        NSInteger indexAssets = numAssets/numShow;
        for (NSInteger i=indexAssets; i>=0; i--) {
            NSIndexSet *indexSet = [NSIndexSet indexSetWithIndexesInRange:NSMakeRange(numShow*i, MIN(numShow, numAssets-numShow*i))];
            DebugLog(@"--%zd-2.1",i);
            [self.assetGroup enumerateAssetsAtIndexes:indexSet options:NSEnumerationReverse usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
...
- (NSArray *)assetsForIndexPath:(NSIndexPath *)path
{
    ...
    if (index+length < self.elcAssets.count) {
        return [self.elcAssets subarrayWithRange:NSMakeRange(index, length)];
    }else{
        return [self.elcAssets subarrayWithRange:NSMakeRange(index, self.elcAssets.count-index)];
//        return nil;
    }
